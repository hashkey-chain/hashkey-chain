...
// the next time the hskchain node restart, we will recovery the msg from this check point.
...
// recoveryChainState tries to recovery consensus chainState from wal when the hskchain node restart.
...
// recoveryMsg tries to recovery consensus msg from wal when the hskchain node restart.
