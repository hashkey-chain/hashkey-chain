...
func TestSimulatedBackend(t *testing.T) {
	...
	tx, _ = types.SignTx(tx, types.NewEIP155Signer(new(big.Int).SetInt64(1337)), key)
	...
	_, isPending, err = sim.TransactionByHash(context.Background(), txHash)
	if err != nil {
		t.Fatalf("error getting transaction with hash: %v,%v", txHash.String(), err)
	}
	if isPending {
...
func TestNewSimulatedBackend_AdjustTimeFail(t *testing.T) {
	...
	signedTx, err := types.SignTx(tx, types.NewEIP155Signer(new(big.Int).SetInt64(1337)), testKey)
	if err != nil {
	...
	signedTx2, err := types.SignTx(tx2, types.NewEIP155Signer(new(big.Int).SetInt64(1337)), testKey)
	if err != nil {
		t.Errorf("could not sign tx: %v", err)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.LAT)}}, 10000000)
	defer sim.Close()
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.LAT*2 + 2e17)}}, 10000000)
	defer sim.Close()
